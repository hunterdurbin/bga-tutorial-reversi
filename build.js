...
$machinestates = ${writer.stringify(statesJSON, true)};`
		...
		fs.writeFileSync('tutorialhunter.action.php',
`<?php
${writer.fileSignature}
class action_tutorialhunter extends APP_GameAction
{
	/** @var tutorialhunter $game */
	...
	public function __default()
	{
		if (self::isArg('notifwindow')) {
			...
			$this->view = "tutorialhunter_tutorialhunter";
...
if (fs.existsSync('source/client/tsconfig.json')) {
	...
	const tsCommand = 'npx tsc -p source/client --outFile tutorialhunter.js' + (builder.watch ? ' -w' : '');
	builder.execCommand(`Typescript compiler: client/define.ts => tutorialhunter.js`, tsCommand);

	// const cookbookModuleRegex = /"@cookbook\/([^"]+)"/g;
	// const searchString = 'define("tutorialhunter", [';
	// const importFixer = () => {
	// 	if (!fs.existsSync('tutorialhunter.js'))
	// 		return;

	// 	const data = fs.readFileSync('tutorialhunter.js', 'utf8');
	// 	const start = data.indexOf(searchString);
	// 	if (start !== -1) {
	// 		const end = start + searchString.length;
	// 		const result = data.substring(0, start) + 'define([' + data.substring(end);
	// 		fs.writeFileSync('tutorialhunter.js', result, 'utf8');
	...
	// builder.watchCommand("Import Generator: source/client/build/imports.d.ts", 'tutorialhunter.js', importFixer);
}

if (fs.existsSync('source/client/tutorialhunter.scss')) {
	builder.execCommand(`SCSS compiler: client/tutorialhunter.scss => tutorialhunter.css`, `npx sass --no-source-map source/client/tutorialhunter.scss tutorialhunter.css` + (builder.watch ? ' --watch' : ''));
}
